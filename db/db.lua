baseurl = 'http://raw.github.com/Mause/CC_Powerplant/master/'

local db_root_dir = '/db'
local rednet_connection_side = "back"
...
function crc(s)
    local bit_band, bit_bxor, bit_rshift, str_byte, str_len = bit.band, bit.bxor, bit.brshift, string.byte, string.len
    ...
    for i = 1, l, 1 do
        crc = bit_bxor(bit_rshift(crc, 8),
            consts[bit_band(bit_bxor(crc, str_byte(s, i)), 0xFF) + 1])
    end
    return bit_bxor(crc, -1)
end


-- sets up the database, duh!
...
function run_server()
    rednet.open(rednet_connection_side)
    while true do
        event, param1, param2, param3 = os.pullEvent()
        if event == "rednet_message" then
            print 'message recieved'
            local client_id = param1
